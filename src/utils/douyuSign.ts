/**
 * 斗鱼直播签名算法
 * 参考 simple_live_core 的实现
 */

/**
 * 生成斗鱼直播签名
 * @param crptext 加密文本 - 可以是 JSON 字符串或已解析的对象
 * @param roomId 房间ID
 * @returns 签名后的参数字符串
 */
export function getSign(crptext: string | object, roomId: string): string {
    try {
        // 解析 crptext - 如果是字符串则解析，如果是对象则直接使用
        let crptextData: any;
        if (typeof crptext === 'string') {
            crptextData = JSON.parse(crptext)
        } else {
            crptextData = crptext;
        }

        const data = crptextData[`room${roomId}`] as string

        // 斗鱼签名算法
        const sign = generateSign(data, roomId)
        return `ct=${data}&pt=${sign}&rid=${roomId}`
    } catch (error) {
 ...
 * @returns 签名字符串
 */
function generateSign(data: string, roomId: string): string {
    // 斗鱼签名算法实现
    // 参考: https://github.com/SocialSisterYi/bilibili-API-collect/issues/878

    // 1. 将 data 和 roomId 组合
    const combined = data + roomId
